...
- 修复：条件编译，只在 Shader Graph + Editor 环境中包含 Editor 路径
*/
#if !defined(SHADERGRAPH_PREVIEW) && defined(SHADERPASS)
	// 只在 Shader Graph 编辑器环境中包含 Editor 路径
	#if defined(UNITY_EDITOR) && defined(SHADER_STAGE_FRAGMENT)
		#include "Packages/com.unity.render-pipelines.universal/Editor/ShaderGraph/Includes/ShaderPass.hlsl"
	#endif
	#if defined(SHADERPASS_FORWARD)
		// Forward pass 不需要 undef
	#else
...
#ifndef SHADERGRAPH_PREVIEW
	uint pixelLightCount = GetAdditionalLightsCount();
	uint meshRenderingLayers = GetMeshRenderingLayer(); // ✅ 修复：使用统一的函数名
